...
async function handleUsers(request, env) {
  ...
  if (request.method === "PATCH") {
    ...
    const existing = await kv.get(`user:${oldUsername}`, "json");
    ...
    if (newUsername !== oldUsername) {
      const clash = await kv.get(`user:${newUsername}`, "json");
      if (clash) return json({ error: `El usuario "${newUsername}" ya existe.` }, 400);
      existing.username = newUsername;
      await kv.put(`user:${newUsername}`, JSON.stringify(existing));
      await kv.delete(`user:${oldUsername}`);
    } else {
      await kv.put(`user:${oldUsername}`, JSON.stringify(existing));
    }
