...
async function handleProvSchedCitas(request, env) {
  ...
  if (request.method === "POST") {
    ...
    // Verifica que el horario siga disponible (evita choques si dos personas reservan al mismo tiempo).
    const existingKeys = await kvListByPrefix(kv, `${scope}:provsched:cita:`);
    for (const key of existingKeys) {
      const c = await kv.get(key, "json");
      if (c && c.status !== "cancelado" && c.date === date && c.time === time) {
  ...
  if (request.method === "PATCH") {
    ...
    if (data.orderNumber) existing.orderNumber = data.orderNumber.trim();
