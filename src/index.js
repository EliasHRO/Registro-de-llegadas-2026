...
async function handleAuth(request, env) {
  ...
  const token = await signToken(user.username, user.role, country, facility);
  return json({ token, username: user.username, role: user.role, country, facility });
}
...
async function handleFormfields(request, env) {
  ...
  if (request.method === "POST") {
    ...
    await kv.put(`${scope}:formfields:custom`, JSON.stringify(clean));
