...
const SUPERUSER_USERNAMES = ["elias"];
...
async function handleAuth(request, env) {
  ...
  if (SUPERUSER_USERNAMES.includes(username)) {
    ...
    const token = await signToken(superUser.username, "superadmin", null, null, true, true, null, null, true);
  ...
  const token = await signToken(user.username, user.role, country, facility, isOwner, false, Array.isArray(user.allowedViews) ? user.allowedViews : null, effectiveMultiScopes, user.canManageProviders === true);
  return json({ token, username: user.username, role: user.role, country, facility, isOwner, global: false, allowedViews: Array.isArray(user.allowedViews) ? user.allowedViews : null, multiScopes: effectiveMultiScopes, canManageProviders: user.canManageProviders === true });
}
...
async function handleUsers(request, env) {
  ...
  if (request.method === "GET") {
    if (auth.global) {
      ...
      for (const s of allScopes()) {
        ...
        for (const key of keys) {
          const u = await kv.get(key, "json");
          if (u) users.push({ username: u.username, role: u.role, createdAt: u.createdAt, scopeKey: s, scopeLabel: scopeLabel(s), allowedViews: Array.isArray(u.allowedViews) ? u.allowedViews : null, multiScopes: Array.isArray(u.multiScopes) ? u.multiScopes : null, canManageProviders: u.canManageProviders === true });
        }
      }
    ...
    if (Array.isArray(auth.multiScopes) && auth.multiScopes.length) {
      // Admin con acceso a varias bodegas (asignado por el superusuario): junta los usuarios de todas ellas,
      // no solo los de su bodega principal — de otro modo, un usuario que él mismo creó en otra bodega
      // asignada le desaparecería de la lista.
      const users = [];
    ...
    for (const key of keys) {
      const u = await kv.get(key, "json");
      if (u) users.push({ username: u.username, role: u.role, createdAt: u.createdAt, allowedViews: Array.isArray(u.allowedViews) ? u.allowedViews : null, multiScopes: Array.isArray(u.multiScopes) ? u.multiScopes : null, canManageProviders: u.canManageProviders === true });
    }
  ...
  if (request.method === "POST") {
    ...
    // Poder eliminar/modificar el Registro de Proveedores: es un permiso que solo el propietario (Owner)
    // puede conceder, y de forma selectiva por usuario — no viene incluido automáticamente por ser Administración.
    const canManageProviders = auth.isOwner && data.canManageProviders === true;
    await kv.put(`${targetScope}:user:${username}`, JSON.stringify({
      username, passwordHash: await hashPassword(password, salt), salt, role, isOwner: false, createdAt: Date.now(), allowedViews, multiScopes, canManageProviders
    }));
    return json({ ok: true });
  }
  ...
  if (request.method === "PATCH") {
    ...
    // Poder eliminar/modificar el Registro de Proveedores: solo el propietario (Owner) puede concederlo
    // o quitarlo, y lo hace por usuario específico, no de forma general para todo el rol Administración.
    if (auth.isOwner && data.canManageProviders !== undefined) {
      existing.canManageProviders = data.canManageProviders === true;
    }
...
async function handleSuggestions(request, env) {
  ...
  if (request.method === "POST") {
    ...
    if (settings) await sendSuggestionNotificationEmail(env, settings, record);
...
// ---------------- /api/verify-order (público, sin login: usado en el check-in para avisar en vivo
// si el número de orden coincide con una cita programada para HOY en esa bodega) ----------------
async function handleVerifyOrder(request, env) {
  ...
  const citaKeys = await idxGet(kv, `${scope}:idx:provsched`);
...
async function handleArrivals(request, env) {
  ...
  if (request.method === "GET") {
    const auth = await verifyStaffToken(request);
    if (!auth) return json({ error: "No autorizado" }, 403);
    // Lee todos los registros de una bodega EN PARALELO (en vez de uno por uno en fila) — con
    // muchos registros acumulados, leerlos de uno en uno podía tardar varios segundos.
    async function fetchArrivalsForScope(s, withScopeInfo) {
      const keys = await idxGet(kv, `${s}:idx:arrival`);
      const vals = await Promise.all(keys.map((key) => kv.get(key, "json")));
      return vals.filter(Boolean).map((v) => (withScopeInfo ? { ...v, scopeKey: s, scopeLabel: scopeLabel(s) } : v));
    }
    let records = [];
    if (auth.global) {
      // Superusuario: junta los registros de TODOS los países y bodegas.
      const perScope = await Promise.all(allScopes().map((s) => fetchArrivalsForScope(s, true)));
      records = perScope.flat();
      ...
      const perScope = await Promise.all(auth.multiScopes.map((s) => fetchArrivalsForScope(s, true)));
      records = perScope.flat();
    } else {
      const scope = storageScope(auth.country, auth.facility);
      records = await fetchArrivalsForScope(scope, false);
    }
  ...
  if (request.method === "POST") {
    ...
    // No permitir un segundo check-in con la misma placa mientras el primero siga activo (esperando o
    // en descarga). Una vez que ese registro se despacha, la misma placa vuelve a quedar disponible
    // para un nuevo check-in (ej. el mismo camión regresando otro día).
    if (plate) {
      const activeArrivalKeys = await idxGet(kv, `${scope}:idx:arrival`);
      for (const key of activeArrivalKeys) {
        const existingArrival = await kv.get(key, "json");
        if (
          existingArrival &&
          existingArrival.status !== "despachado" &&
          (existingArrival.plate || "").trim().toUpperCase() === plate.toUpperCase()
        ) {
          return json({ error: `La placa ${plate} ya tiene un registro activo en la plataforma. Debe completarse el despacho antes de poder registrar un nuevo check-in con la misma placa.` }, 409);
        }
      }
    }

    // No permitir un segundo check-in con la MISMA orden de compra, sin importar el estado del
    // registro anterior (a diferencia de la placa, aquí no se libera al despachar) — cada orden de
    // compra representa una entrega única. Si de verdad hay más de un camión para la misma orden,
    // se debe diferenciarlos con un sufijo (ej. "L100589-1", "L100589-2").
    if (orderNumber) {
      const allArrivalKeys = await idxGet(kv, `${scope}:idx:arrival`);
      for (const key of allArrivalKeys) {
        const existingArrival = await kv.get(key, "json");
        if (
          existingArrival &&
          (existingArrival.orderNumber || "").trim().toLowerCase() === orderNumber.toLowerCase()
        ) {
          return json({ error: `La orden de compra ${orderNumber} ya fue registrada anteriormente por otro conductor (${existingArrival.driverName || "sin nombre"}). Verifique el número de orden — si de verdad son camiones distintos para la misma orden, agregue un sufijo para diferenciarlos (ej. ${orderNumber}-2).` }, 409);
        }
      }
    }
    ...
    if (orderNumber) {
      ...
      for (const key of citaKeys) {
        const cita = await kv.get(key, "json");
        if (!cita || cita.status === "cancelado") continue;
        if ((cita.orderNumber || "").trim().toLowerCase() !== orderNumber.toLowerCase()) continue;
        if (cita.linkedArrivalId) {
          // La cita dice estar vinculada — pero si ese registro de llegada ya no existe (se borró sin
          // pasar por el flujo normal, o quedó huérfano por cualquier otra razón), se libera sola para
          // poder vincularse con este nuevo check-in en vez de quedarse pegada para siempre.
          const stillExists = await kv.get(cita.linkedArrivalId, "json");
    ...
    if (settings) await sendArrivalNotificationEmail(env, settings, record);
  ...
  if (request.method === "PATCH") {
    ...
    if (data.orderNumber !== undefined) {
      ...
      if (newOrderNumber !== (existing.orderNumber || "")) {
        ...
        // Si esa cita ya estaba vinculada a OTRO registro, se prioriza esta corrección manual del
        // personal (es una señal explícita de que esta llegada sí es la que corresponde a esa cita)
        // y se libera el vínculo anterior, sin importar si ese otro registro sigue existiendo o no.
        if (newOrderNumber && scope) {
          const citaKeys = await kvListByPrefix(kv, `${scope}:provsched:cita:`);
          for (const key of citaKeys) {
            const cita = await kv.get(key, "json");
            if (!cita || cita.status === "cancelado") continue;
            if ((cita.orderNumber || "").trim().toLowerCase() !== newOrderNumber.toLowerCase()) continue;
            if (cita.linkedArrivalId && cita.linkedArrivalId !== data.id) {
              const previouslyLinked = await kv.get(cita.linkedArrivalId, "json");
              if (previouslyLinked && previouslyLinked.linkedCitaId === key) {
                delete previouslyLinked.linkedCitaId;
                delete previouslyLinked.punctuality;
                delete previouslyLinked.punctualityMinutes;
                await kv.put(cita.linkedArrivalId, JSON.stringify(previouslyLinked));
              }
            }
            {
...
// Igual que sendNewCitaEmail, pero para las notificaciones internas de "llegadas" y "sugerencias"
// (Configuración → Notificaciones). Se manda por Resend en vez de por el webhook de Zapier/Make, porque
// Resend da 3,000 correos gratis al mes — muchísimo más margen que las 100 tareas/mes del plan gratis
// de Zapier, que con un volumen de llegadas alto se agota casi de inmediato.
function escapeHtmlEmail(str) {
  if (str === null || str === undefined) return "";
  return String(str).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
}

async function sendArrivalNotificationEmail(env, settings, record) {
  if (!env.RESEND_API_KEY || !env.EMAIL_FROM) return;
  const to = (settings.notifyEmail || "").split(",").map((s) => s.trim()).filter(Boolean);
  if (!to.length) return;

  const html = `
    <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;">
      <div style="background:#1C1F26;color:#fff;padding:18px 22px;border-radius:12px 12px 0 0;">
        <div style="font-size:11px;font-weight:700;letter-spacing:.05em;text-transform:uppercase;opacity:.85;">Torre de Control</div>
        <div style="font-size:18px;font-weight:800;margin-top:4px;">Nueva llegada registrada</div>
      </div>
      <div style="border:1px solid #E1E4E9;border-top:none;border-radius:0 0 12px 12px;padding:22px;">
        <table style="width:100%;border-collapse:collapse;font-size:14px;">
          <tr><td style="padding:6px 0;color:#6B7280;">Proveedor</td><td style="padding:6px 0;font-weight:700;">${escapeHtmlEmail(record.provider)}</td></tr>
          <tr><td style="padding:6px 0;color:#6B7280;">Motorista</td><td style="padding:6px 0;font-weight:600;">${escapeHtmlEmail(record.driverName)}</td></tr>
          ${record.phone ? `<tr><td style="padding:6px 0;color:#6B7280;">Teléfono</td><td style="padding:6px 0;font-weight:600;">${escapeHtmlEmail(record.phone)}</td></tr>` : ""}
          ${record.plate ? `<tr><td style="padding:6px 0;color:#6B7280;">Placa</td><td style="padding:6px 0;font-weight:600;">${escapeHtmlEmail(record.plate)}</td></tr>` : ""}
          ${record.orderNumber ? `<tr><td style="padding:6px 0;color:#6B7280;">Orden de compra</td><td style="padding:6px 0;font-weight:600;">${escapeHtmlEmail(record.orderNumber)}</td></tr>` : ""}
          <tr><td style="padding:6px 0;color:#6B7280;">Hora de registro</td><td style="padding:6px 0;font-weight:600;">${escapeHtmlEmail(record.time)}</td></tr>
        </table>
      </div>
    </div>
  `;
  try {
    await fetch("https://api.resend.com/emails", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${env.RESEND_API_KEY}` },
      body: JSON.stringify({ from: env.EMAIL_FROM, to, subject: `Nueva llegada · ${record.provider || ""}`, html })
    });
  } catch (e) { /* nunca debe tumbar el registro de la llegada */ }
}

async function sendSuggestionNotificationEmail(env, settings, record) {
  if (!env.RESEND_API_KEY || !env.EMAIL_FROM) return;
  const to = (settings.suggestionsEmail || settings.notifyEmail || "").split(",").map((s) => s.trim()).filter(Boolean);
  if (!to.length) return;
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;">
      <div style="background:#1C1F26;color:#fff;padding:18px 22px;border-radius:12px 12px 0 0;">
        <div style="font-size:11px;font-weight:700;letter-spacing:.05em;text-transform:uppercase;opacity:.85;">Torre de Control</div>
        <div style="font-size:18px;font-weight:800;margin-top:4px;">Nueva sugerencia</div>
      </div>
      <div style="border:1px solid #E1E4E9;border-top:none;border-radius:0 0 12px 12px;padding:22px;">
        <p style="margin:0 0 10px;color:#6B7280;font-size:12.5px;">De: ${escapeHtmlEmail(record.from)}</p>
        <p style="margin:0;color:#1C1F26;white-space:pre-wrap;">${escapeHtmlEmail(record.message)}</p>
      </div>
    </div>
  `;
  try {
    await fetch("https://api.resend.com/emails", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${env.RESEND_API_KEY}` },
      body: JSON.stringify({ from: env.EMAIL_FROM, to, subject: `Nueva sugerencia de ${record.from || "un usuario"}`, html })
    });
  } catch (e) { /* nunca debe tumbar el envío de la sugerencia */ }
}
...
// Normaliza un nombre de empresa para comparar duplicados: minúsculas, sin puntos ni comas,
// espacios de más colapsados — así "Ternium Internacional Guatemala S.A." y
// "Ternium Internacional Guatemala S.A" (o con espacios extra) se detectan como la misma empresa.
...
async function handleProviderAuth(request, env) {
  ...
  if (data.action === "register") {
    ...
    // Evita duplicar la misma empresa con una cuenta distinta — compara el nombre "normalizado"
    // (sin mayúsculas, sin puntos ni espacios de más) para detectar coincidencias como
    // "Ternium Internacional Guatemala S.A." vs "Ternium Internacional Guatemala S.A".
    const normalizedNewName = normalizeCompanyName(companyName);
    const providerKeys = await kvListByPrefix(kv, `${country}:provider:`);
    for (const pk of providerKeys) {
      const p = await kv.get(pk, "json");
      if (p && normalizeCompanyName(p.companyName) === normalizedNewName) {
        return json({ error: "Empresa ya registrada imposible registrar" }, 409);
      }
    }
...
async function handleProviders(request, env) {
  ...
  if (request.method === "GET") {
    ...
    for (const key of keys) {
      ...
      if (p) providers.push({ username: p.username, companyName: p.companyName, contactName: p.contactName, phone: p.phone, email: p.email, extraEmails: Array.isArray(p.extraEmails) ? p.extraEmails : [], customFields: p.customFields || {}, createdAt: p.createdAt || null, createdBy: p.createdBy || null });
    }
  ...
  if (request.method === "PATCH") {
    // Consultar el registro de un proveedor es para Owner + Administración. Modificar la mayoría de
    // los datos (lo que el proveedor llenó desde su login) es exclusivo del propietario, O de un
    // Administración al que el propietario le haya concedido el permiso "canManageProviders" —
    // los "correos adicionales" los puede administrar cualquier Administración de todos modos, ya que
    // es una lista que agrega el personal, no algo que el proveedor haya llenado.
    ...
    const hasFullAccess = auth.isOwner || auth.canManageProviders === true;
    if (!hasFullAccess) {
      const onlyAllowedKeys = ["username", "extraEmails"];
      const hasOtherFields = Object.keys(data).some((k) => !onlyAllowedKeys.includes(k));
      if (hasOtherFields) return json({ error: "No tienes permiso para modificar los demás datos del proveedor. Pide al usuario propietario que te conceda acceso desde Configuración → Usuarios." }, 403);
    }
    const country = VALID_COUNTRIES.includes(auth.country) ? auth.country : "GT";
    ...
    if (Array.isArray(data.extraEmails)) {
      record.extraEmails = data.extraEmails
        .map((e) => ({
          email: (e && e.email || "").trim(),
          name: (e && e.name || "").trim() || null,
          cc: !!(e && e.cc)
        }))
  ...
  if (request.method === "DELETE") {
    // Eliminar un proveedor por completo (útil para limpiar registros duplicados) es exclusivo del
    // propietario, o de un Administración al que el propietario le haya concedido "canManageProviders".
    // Pide la propia contraseña de quien elimina, para confirmar identidad antes de un borrado permanente.
    if (!auth.isOwner && auth.canManageProviders !== true) {
      return json({ error: "No tienes permiso para eliminar proveedores. Pide al usuario propietario que te conceda acceso desde Configuración → Usuarios." }, 403);
    }
    const data = await readJson(request);
    if (!data || !data.username || !data.password) {
      return json({ error: "Falta el usuario del proveedor o tu contraseña de confirmación" }, 400);
    }
    const userRecord = await getUserRecord(kv, auth);
    if (!userRecord) return json({ error: "Usuario no encontrado" }, 404);
    const passwordOk = await verifyPassword(data.password, userRecord.salt, userRecord.passwordHash);
    if (!passwordOk) return json({ error: "Contraseña incorrecta" }, 401);

    const country = VALID_COUNTRIES.includes(auth.country) ? auth.country : "GT";
    const key = providerKey(country, data.username);
    const existing = await kv.get(key, "json");
    if (!existing) return json({ error: "Proveedor no encontrado" }, 404);
    await kv.delete(key);
...
async function handleProvSchedCitas(request, env) {
  ...
  if (request.method === "GET") {
    // Lee todas las citas de una bodega EN PARALELO (en vez de una por una en fila) — con muchas
    // citas acumuladas, leerlas de una en una podía tardar varios segundos; en paralelo tarda
    // prácticamente lo mismo que leer una sola.
    async function fetchCitasForScope(s, withScopeInfo) {
      const keys = await idxGet(kv, `${s}:idx:provsched`);
      const records = await Promise.all(keys.map((key) => kv.get(key, "json")));
      return records.filter(Boolean).map((c) => (withScopeInfo ? { ...c, scopeKey: s, scopeLabel: scopeLabel(s) } : c));
    }
    let citas = [];
    if (auth.global) {
      const perScope = await Promise.all(allScopes().map((s) => fetchCitasForScope(s, true)));
      citas = perScope.flat();
    } else if (Array.isArray(auth.multiScopes) && auth.multiScopes.length) {
      const perScope = await Promise.all(auth.multiScopes.map((s) => fetchCitasForScope(s, true)));
      citas = perScope.flat();
      ...
      const scopes = COUNTRY_FACILITIES[auth.country].map((f) => `${auth.country}:${f.code}`);
      const perScope = await Promise.all(scopes.map((s) => fetchCitasForScope(s, true)));
      citas = perScope.flat();
    } else {
      const scope = storageScope(auth.country, auth.facility);
      citas = await fetchCitasForScope(scope, false);
    }
  ...
  if (request.method === "PATCH") {
    ...
    // Cambiar el estado manualmente: para corregir casos donde el flujo normal de check-in no se usó
    // (ej. el transportista llegó y se descargó, pero nunca se hizo el check-in por QR, así que la cita
    // se quedó pegada en "Programado" sin forma de reflejar lo que realmente pasó).
    if (data.setStatus !== undefined) {
      const validManualStatuses = ["programado", "en_espera", "en_descarga", "completado"];
      if (!validManualStatuses.includes(data.setStatus)) {
        return json({ error: "Estado inválido" }, 400);
      }
      if (auth.role !== "admin" && auth.role !== "superadmin") {
        return json({ error: "Solo un usuario con rol Administración puede cambiar el estado manualmente" }, 403);
      }
      history.push({ action: "estado_manual", from: existing.status, to: data.setStatus, at: Date.now(), by: auth.username });
      existing.status = data.setStatus;
  ...
  if (request.method === "DELETE") {
    ...
    if (data.permanent === true) {
      // Borrado real y definitivo: solo toca la clave exacta de este registro y su entrada en el
      // índice — no puede afectar ningún otro registro ni programación.
      await kv.delete(data.id);
      await idxRemove(kv, `${scope}:idx:provsched`, data.id);
      return json({ ok: true, deleted: true });
    }
  ...
  async fetch(request, env) {
    ...
    if (path.startsWith("/api/")) {
      try {
        ...
        if (path === "/api/verify-order") return await handleVerifyOrder(request, env);
