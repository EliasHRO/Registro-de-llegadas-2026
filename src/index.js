...
async function handleArrivals(request, env) {
  ...
  if (request.method === "POST") {
    ...
    // Vincula automáticamente con una cita programada que tenga la misma orden de compra (llave principal).
    // Esto permite que "Programación de Proveedores" refleje en vivo el estado real del registro de llegada.
    if (orderNumber) {
      ...
      for (const key of citaKeys) {
        ...
        if (
          ...
          cita.linkedArrivalId = id;
          cita.status = "en_espera";
...
async function handleProvSchedCitas(request, env) {
  ...
  if (request.method === "DELETE") {
    // Solo el rol Administración (o el superusuario) puede cancelar una programación, y requiere la contraseña de compras.
    if (auth.role !== "admin" && auth.role !== "superadmin") {
      return json({ error: "Solo un usuario con rol Administración puede eliminar una programación" }, 403);
    }
    const data = await readJson(request);
    if (!data || !data.id) return json({ error: "Falta id" }, 400);
...
async function runScheduledBackup(env, period) {
  ...
  const r2Key = period === "semanal" ? `backups/semanal/${dateKey}.json` : `backups/diario/${dateKey}.json`;
  await env.BACKUPS.put(r2Key, JSON.stringify(backup), { httpMetadata: { contentType: "application/json" } });
...
// ---------------- /api/backups (listar y descargar respaldos guardados en R2, solo superusuario) ----------------
async function handleBackups(request, env) {
  ...
  if (request.method === "POST") {
    // Generar un respaldo manual ahora mismo (además de los automáticos programados).
  ...
  if (downloadKey) {
    if (!downloadKey.startsWith("backups/")) return json({ error: "Clave inválida" }, 400);
    const obj = await env.BACKUPS.get(downloadKey);
    if (!obj) return json({ error: "Respaldo no encontrado" }, 404);
    return new Response(obj.body, {
      headers: { "Content-Type": "application/json", "Content-Disposition": `attachment; filename="${downloadKey.split("/").pop()}"`, ...CORS_HEADERS }
    });
  }

  const listed = await env.BACKUPS.list({ prefix: "backups/" });
  const items = listed.objects.map((o) => ({ key: o.key, size: o.size, uploaded: o.uploaded }));
