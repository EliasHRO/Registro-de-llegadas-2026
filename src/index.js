    ...
    if (orderNumber) {
      ...
      for (const key of citaKeys) {
        ...
        if (
          ...
          const punctuality = computePunctuality(cita.date, cita.time, date, time);
          if (punctuality) {
...
async function handleProvSchedCitas(request, env) {
  ...
  if (request.method === "GET") {
    ...
    if (auth.global) {
      ...
      // País con varias bodegas (ej. Guatemala: CD Atlas y CD Santa Elena): cualquier admin de ese país
    ...
    if (data.date || data.time) {
      ...
      if (newDate !== existing.date || newTime !== existing.time) {
        ...
        for (const key of existingKeys) {
          ...
          const c = await kv.get(key, "json");
          if (c && isActiveCitaStatus(c.status) && c.date === newDate && c.time === newTime) {
            return json({ error: "Ese horario ya no está disponible. Elige otro." }, 409);
          }
        }
