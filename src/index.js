...
// Superusuario global: mismo usuario/contraseña sin importar el país/bodega, con acceso a todos los países.
const SUPERUSER_USERNAME = "elias";
...
async function handleAuth(request, env) {
  ...
  // ---- Superusuario global (Elias): mismo usuario/contraseña en cualquier país/bodega,
  // con acceso y permisos de eliminar sobre TODOS los países y bodegas. ----
  if (username === SUPERUSER_USERNAME) {
    let superUser = await kv.get(`${GLOBAL_SCOPE}:user:${SUPERUSER_USERNAME}`, "json");
    if (!superUser) {
      const salt = makeSalt();
      superUser = {
      ...
      await kv.put(`${GLOBAL_SCOPE}:user:${SUPERUSER_USERNAME}`, JSON.stringify(superUser));
    }
    ...
    const token = await signToken(superUser.username, "superadmin", country, facility, true, true);
    return json({ token, username: superUser.username, role: "superadmin", country, facility, isOwner: true, global: true });
  }
  ...
  const token = await signToken(user.username, user.role, country, facility, isOwner, false);
  return json({ token, username: user.username, role: user.role, country, facility, isOwner, global: false });
}
...
async function handleUsers(request, env) {
  ...
  // Resuelve en qué scope (país/bodega) opera esta solicitud de escritura.
  // El superusuario puede indicar data.scope para editar/eliminar usuarios de cualquier país; los demás roles siempre operan en su propio scope.
  function resolveTargetScope(data) {
    if (auth.global && data && data.scope) {
      if (!allScopes().includes(data.scope)) return null;
      return data.scope;
    }
    return scope;
  }
    ...
    if (auth.global) {
      ...
      for (const s of allScopes()) {
        ...
        for (const key of keys) {
          const u = await kv.get(key, "json");
          if (u) users.push({ username: u.username, role: u.role, createdAt: u.createdAt, scopeKey: s, scopeLabel: scopeLabel(s) });
        }
      }
  ...
  if (request.method === "POST") {
    const data = await readJson(request);
    if (!data) return json({ error: "JSON inválido" }, 400);
    const targetScope = resolveTargetScope(data) || scope;
    const username = (data.username || "").trim().toLowerCase();
    const password = data.password || "";
    const validRoles = ["admin", "operador", "asistente", "viewer", "conductor"];
    const role = validRoles.includes(data.role) ? data.role : "viewer";
    if (username === SUPERUSER_USERNAME) return json({ error: "Ese nombre de usuario está reservado" }, 400);
  ...
  if (request.method === "PATCH") {
    const data = await readJson(request);
    if (!data) return json({ error: "JSON inválido" }, 400);
    const targetScope = resolveTargetScope(data);
    ...
    if (data.newUsername) {
      ...
      if (newUsername === SUPERUSER_USERNAME) return json({ error: "Ese nombre de usuario está reservado" }, 400);
