...
async function handleAuth(request, env) {
  ...
  const token = await signToken(user.username, user.role, country, facility, isOwner, false, Array.isArray(user.allowedViews) ? user.allowedViews : null);
  return json({ token, username: user.username, role: user.role, country, facility, isOwner, global: false, allowedViews: Array.isArray(user.allowedViews) ? user.allowedViews : null });
}

const NAV_VIEW_KEYS = ["dashboard", "calendar", "provsched", "insights-arrivals", "suggest", "config"];
...
async function handleUsers(request, env) {
  ...
  if (request.method === "GET") {
    if (auth.global) {
      ...
      for (const s of allScopes()) {
        ...
        for (const key of keys) {
          const u = await kv.get(key, "json");
          if (u) users.push({ username: u.username, role: u.role, createdAt: u.createdAt, scopeKey: s, scopeLabel: scopeLabel(s), allowedViews: Array.isArray(u.allowedViews) ? u.allowedViews : null });
        }
      }
    ...
    for (const key of keys) {
      const u = await kv.get(key, "json");
      if (u) users.push({ username: u.username, role: u.role, createdAt: u.createdAt, allowedViews: Array.isArray(u.allowedViews) ? u.allowedViews : null });
    }
  ...
  if (request.method === "POST") {
    ...
    const salt = makeSalt();
    // Qué secciones puede ver este usuario: solo el superusuario global puede definirlo al crear la cuenta.
    ...
    await kv.put(`${targetScope}:user:${username}`, JSON.stringify({
      username, passwordHash: await hashPassword(password, salt), salt, role, isOwner: false, createdAt: Date.now(), allowedViews
    }));
    return json({ ok: true });
  }
  ...
  if (request.method === "PATCH") {
    ...
    // Qué secciones puede ver este usuario: solo el superusuario global puede modificarlo.
...
async function handleProvSchedConfig(request, env) {
  ...
  if (request.method === "GET") {
    const auth = await verifyToken(getTokenFromRequest(request));
    if (!auth) return json({ error: "No autorizado" }, 403);
    const scope = auth.global ? resolvePublicScope(request) : storageScope(auth.country, auth.facility);
    if (!scope) return json({ error: "País/bodega inválido o no especificado" }, 400);
...
async function handleProvSchedCitas(request, env) {
  ...
  if (request.method === "PATCH") {
    ...
    if (auth.global) {
      scope = scopeFromRecordId(data.id);
      if (!scope) return json({ error: "Id inválido" }, 400);
    } else {
      scope = storageScope(auth.country, auth.facility);
      if (!data.id.startsWith(`${scope}:`)) return json({ error: "No autorizado" }, 403);
    }
