  ...
  for (const facilityCode of facilities) {
    ...
    for (const key of keys) {
      ...
      if (c.linkedArrivalId) {
        ...
        if (a) arrival = { status: a.status || "esperando", dock: a.dock || null };
      }
      orders.push({
...
export default {
  async fetch(request, env) {
    ...
    // Ruta limpia /proveedor: portal de proveedores (registro, login y tracking de sus órdenes).
    if (path === "/proveedor") {
      const providerUrl = new URL(request.url);
      providerUrl.pathname = "/provider.html";
      return env.ASSETS.fetch(new Request(providerUrl, request));
    }
