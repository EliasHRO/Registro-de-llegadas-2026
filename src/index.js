...
async function handleProviders(request, env) {
  ...
  if (request.method === "PATCH") {
    ...
    // los datos (lo que el proveedor llenó desde su login) es exclusivo del propietario — PERO los
    // "correos adicionales" sí los puede administrar el rol Administración también, ya que es una
    // lista que agrega el personal, no algo que el proveedor haya llenado.
    ...
    if (!auth.isOwner) {
      ...
      if (hasOtherFields) return json({ error: "Solo el usuario propietario puede modificar los demás datos del proveedor" }, 403);
    }
    const country = VALID_COUNTRIES.includes(auth.country) ? auth.country : "GT";
