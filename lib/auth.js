...
export async function signToken(username, role, country, facility) {
  const payload = JSON.stringify({ u: username, r: role, c: country, f: facility || null, exp: Date.now() + 12 * 3600 * 1000 });
...
export async function verifyToken(token) {
  ...
  try {
    ...
    return { username: payload.u, role: payload.r, country: payload.c, facility: payload.f || null };
  } catch {
    return null;
  }
}
