...
export async function signToken(username, role, country, facility, isOwner, global, allowedViews, multiScopes, canManageProviders) {
  const payload = JSON.stringify({ u: username, r: role, c: country, f: facility || null, o: !!isOwner, g: !!global, av: Array.isArray(allowedViews) ? allowedViews : null, ms: Array.isArray(multiScopes) ? multiScopes : null, cmp: !!canManageProviders, exp: Date.now() + 12 * 3600 * 1000 });
...
export async function verifyToken(token) {
  ...
  try {
    ...
    return { username: payload.u, role: payload.r, country: payload.c, facility: payload.f || null, isOwner: !!payload.o, global: !!payload.g, allowedViews: Array.isArray(payload.av) ? payload.av : null, multiScopes: Array.isArray(payload.ms) ? payload.ms : null, canManageProviders: !!payload.cmp };
  } catch {
    return null;
  }
}
...
// ---------------- Índices baratos (evitan kv.list()) ----------------
// El plan gratis de Cloudflare KV solo permite 1,000 operaciones list() por día, contra 100,000
// lecturas normales — por eso, para las pantallas que se refrescan solas cada pocos segundos
// (Llegadas, Programación de Citas, Portal de Proveedores), mantenemos un índice (una simple
// lista de claves) guardado bajo una única clave, y lo leemos con kv.get() en vez de listar.
export async function idxGet(kv, indexKey) {
  try {
    const idx = await kv.get(indexKey, "json");
    return Array.isArray(idx) ? idx : [];
  } catch (e) {
    return [];
  }
}
export async function idxAdd(kv, indexKey, itemKey) {
  const idx = await idxGet(kv, indexKey);
  if (!idx.includes(itemKey)) {
    idx.push(itemKey);
    await kv.put(indexKey, JSON.stringify(idx));
  }
}
export async function idxRemove(kv, indexKey, itemKey) {
  const idx = await idxGet(kv, indexKey);
  const filtered = idx.filter((k) => k !== itemKey);
