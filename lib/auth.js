...
export async function signToken(username, role, country, facility, isOwner, global, allowedViews) {
  const payload = JSON.stringify({ u: username, r: role, c: country, f: facility || null, o: !!isOwner, g: !!global, av: Array.isArray(allowedViews) ? allowedViews : null, exp: Date.now() + 12 * 3600 * 1000 });
...
export async function verifyToken(token) {
  ...
  try {
    ...
    return { username: payload.u, role: payload.r, country: payload.c, facility: payload.f || null, isOwner: !!payload.o, global: !!payload.g, allowedViews: Array.isArray(payload.av) ? payload.av : null };
  } catch {
    return null;
  }
}
...
export function scopeFromRecordId(id) {
  const markers = [":arrival:", ":archived:"];
